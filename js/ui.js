...
import { applySearch, applyFiltersCore, applySort } from './fss.js'
...
dlog('✅ main.js loaded')
window.DEFAULT_NOTE_TEMPLATE = `FormType:
New House:
Old House:`
...
let lastVisibleSerial =
  Number(localStorage.getItem('lastVisibleSerial')) || null
...
*/
  ...
  window.SHOW_ONLY_NOTED = localStorage.getItem('sidebar_swNotes') === '1'
  ...
  document
    ...
    ?.addEventListener('click', window.bulkUpdateAddressForAllVoters)

  document
    .getElementById('fixSelectedAddressBtn')
    ?.addEventListener('click', window.confirmFixSelectedAddress)
})
...
window.processData = function processData() {
  ...
  Object.keys(voterData).forEach(h => {
    Object.entries(voterData[h] || {}).forEach(([key, p]) => {
      // 🔥 HARD RULE:
      // Firebase active হলে → offline_* NEVER include
      if (key.startsWith('offline_') && window.USING_FIREBASE_DATA === true) {
        return
      }

      // 🔥 Safety: invalid data skip
...
function generateColors() {
  const houses = [...new Set(window.allPeople.map(p => p.house))].sort(
    sortHouseASC
  )
  houses.forEach((h, i) => {
...
window.updateStats = function updateStats(list = []) {
  ...
  // Total MUST come from rendered list
  ...
  // 👤 Gender
  statMale.textContent = list.filter(
    p => normalizeGender(p.gender) === 'Male'
  ).length

  statFemale.textContent = list.filter(
    p => normalizeGender(p.gender) === 'Female'
  ).length

  // ❌ duplicateBYP stat intentionally unused
  ...
  if (statGeneralEl) {
    statGeneralEl.textContent = list.filter(
      p => (p.caste || 'General') === 'General'
    ).length
  }
  ...
  if (statVerifiedEl) {
    statVerifiedEl.textContent = list.filter(p => p.verified === true).length
  }

  const statUnverifiedEl = document.getElementById('statUnverified')

  if (statUnverifiedEl) {
    statUnverifiedEl.textContent = list.filter(p => !p.verified).length
  }

  // ☪️ Muslim verification split
  const statVerifiedMuslimEl = document.getElementById('statVerifiedMuslim')

  if (statVerifiedMuslimEl) {
    statVerifiedMuslimEl.textContent = list.filter(
      p => p.caste === 'Muslim' && p.verified === true
    ).length
  }

  const statUnverifiedMuslimEl = document.getElementById('statUnverifiedMuslim')

  if (statUnverifiedMuslimEl) {
    statUnverifiedMuslimEl.textContent = list.filter(
      p => p.caste === 'Muslim' && !p.verified
    ).length
  }

  // 🔁 DUPLICATE SERIAL (SINGLE SOURCE)
  const dupSerialCount = list.filter(p => p.duplicateSerial === true).length

  const statDupSerialEl = document.getElementById('statDupSerial')
...
if (sidebarUploadPdfBtn) {
  sidebarUploadPdfBtn.addEventListener('click', () => {
    window.open('https://voterlist.onrender.com', '_blank')
  })
}
...
if (sidebarHolding) {
  sidebarHolding.addEventListener('click', () => {
    window.open('https://avoter.onrender.com', '_blank')
  })
}
...
sortBy?.addEventListener('change', () => {
  ...
  applySort({
    mode: sortBy.value,
    data: window.lastRenderedList.length ? window.lastRenderedList : allPeople,
    renderResults,
    setPage: () => (currentPage = 1)
  })
})
...
window.editVoter = function (house, key) {
  let voter = allPeople.find(
    p => String(p.key) === String(key) && String(p.house) === String(house)
  )
...
function updateStickyHeaderVisibility() {
  ...
  const hasVisibleContent = [...header.querySelectorAll('*')].some(el => {
...
window.applyShowNotes = function (on) {
  ...
  localStorage.setItem('sidebar_swNotes', on ? '1' : '0')
...
function scrollToLastVisibleSerial() {
  const serial = Number(localStorage.getItem('lastVisibleSerial'))
  ...
  cards.forEach(card => {
    ...
    const num = Number(pill.innerText.replace('#', ''))
